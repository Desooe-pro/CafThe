...
router.get("/produits/fiche/:id", (req, res) => {
    db.query(`SELECT A.Designation_Article, A.Prix_unitaire_Article, A.Description_Article, M.Designation_Mesure
              FROM article AS A
...
router.get("/clients/like/:nom", (req, res) => {
    db.query(`SELECT * FROM client WHERE Nom_Prenom_Client LIKE "%${req.params.nom}%"`, (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
            return(res.status(404).json({message : `Client ${req.params.nom} non trouvé`}))
        }
        res.json(result);
    });
});
    ...
    db.query(`SELECT * FROM client WHERE Identifiant_Client = ?`, [req.params.id], (error, result) => {
        ...
        if (result.length < 1){
            ...
            bcrypt.hash(pw, 10, (error, hash) => {
                ...
                db.query(`UPDATE client SET MDP_Client = ? WHERE Identifiant_Client = ?`, [hash, req.params.id], (error, result) => {
                    if (error){
                        return(res.status(500).json({message : "Erreur lors de la modification du mot de passe"}))
                    }
                    res.status(201).json({message : "Modification du mot de passe réussie", nouveau_mot_de_passe : pw})
                })
            })
        }
    })
...
router.get("/clients/login/connexion", (req, res) => {
    ...
    db.query(`SELECT * FROM client WHERE Mail_Client = ?`, [mail], (error, result) => {
        ...
        bcrypt.compare(pw, result[0].MDP_Client, (error,isMatch) => {
            if (error){
                return(res.status(500).json({message: "Authentification ratée", error: error}));
            }
            if (isMatch){
                return(res.status(400).json({message: "Authentification réussi"}));
            }
        })
    });
});
...
router.get("/vendeur/:id", (req, res) => {
    db.query(`SELECT * FROM vendeur WHERE Id_Vendeur = ?`, [req.params.id], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
            return(res.status(404).json({message : `Vendeur ${req.params.id} non trouvé`}))
        }
        res.json(result);
    });
});
...
router.get("/vendeur/like/:id", (req, res) => {
    db.query(`SELECT * FROM vendeur WHERE Id_Vendeur LIKE "%${req.params.id}%"`, (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
...
router.post("/vendeur/register", (req, res) => {
    ...
    // Vérifie que l'adresse mail n'est pas déjà utilisé
    db.query("SELECT * FROM vendeur WHERE Mail_Vendeur = ?", [Mail], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length > 0){
            ...
            bcrypt.hash(MDP, 10, (error, hash) => {
                ...
                db.query(
                    ...
                    (error, result) => {
                        ...
                        res.status(201).json({message : "Inscription réussie", vendeur_id : result.insertId})
                    })
            })
        }
    })
...
router.get("/paniers/:id", (req, res) => {
    db.query(`SELECT * FROM panier WHERE Id_Panier = ?`, [req.params.id], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
...
router.get("/paniers/client/:id", (req, res) => {
    db.query(`SELECT * FROM panier WHERE Identifiant_Client = ?`, [req.params.id], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
...
router.get("/paniers/client/open/:id", (req, res) => {
    db.query(`SELECT * FROM panier WHERE Identifiant_Client = ? AND Status = "Ouvert"`, [req.params.id], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
...
router.get("/CB/client/:id", (req, res) => {
    db.query(`SELECT *FROM carte_bancaire AS C WHERE C.Identifiant_Client = ?`, [req.params.id], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
...
router.get("/CB/client/like/:nom", (req, res) => {
    db.query(`SELECT * FROM carte_bancaire AS C WHERE C.Identifiant_Client = (SELECT CL.Identifiant_Client FROM client AS CL WHERE CL.Nom_Prenom_Client LIKE "%${req.params.nom}%")`,
        (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
...
router.post("/CB/register", (req, res) => {
    const {Type_CB, Numero_CB, Date_expiration_CB, Nom_CB, Identifiant_Client} = req.body;
    console.log(Type_CB, Numero_CB, Date_expiration_CB, Nom_CB, Identifiant_Client)
    let New_Numero_CB = Numero_CB.slice(0,4) + "XXXXXXXX" + Numero_CB.slice(-4);
    console.log(New_Numero_CB);

    // Vérifie que l'adresse mail n'est pas déjà utilisé
    db.query("SELECT * FROM carte_bancaire WHERE Numero_CB = ?", [New_Numero_CB], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length > 0){
            return(res.status(400).json({message : "Cette carte bancaire est déjà enregistrée"}));
        } else {
            db.query(
                "INSERT INTO carte_bancaire (Type_CB, Numero_CB, Date_expiration_CB, Nom_CB, Identifiant_Client) VALUES (?, ?, ?, ?, ?)",
                [Type_CB, New_Numero_CB, Date_expiration_CB, Nom_CB, Identifiant_Client],
                (error, result) => {
                    if (error){
                        console.log(error)
                        return(res.status(500).json({message : "Erreur lors de l'inscription"}));
                    }
                    res.status(201).json({message : "Inscription réussie", CB_ID : result.insertId})
                })
        }
    })
})
...
router.post("/lignedepanier/register", (req, res) => {
    ...
    db.query(`SELECT * FROM panier WHERE Id_Panier = ?`, [Id_Panier], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
            ...
            db.query(`SELECT * FROM article WHERE Id_Article = ?`, [Id_Article], (error, result) => {
                if (error){
                    return(res.status(500).json({message : "Erreur du serveur"}));
                }
                if (result.length === 0){
                    ...
                    db.query(`INSERT INTO ligne_de_panier (Id_Panier, Quantite_Ligne_de_panier, Prix_unitaire_Ligne_de_panier, Id_Article) VALUES (?, ?, 0, ?)`,
                        [Id_Panier, Quantite_Ligne_de_panier, Id_Article],
                        (error, result) => {
                            if (error){
                                return(res.status(500).json({message: "Erreur lors de la création de la ligne de panier"}))
                            }
                            res.status(201).json({message : "Inscription réussie", Ligne_Id : result.insertId})
                        })
                }
            })
        }
...
router.post("/commande/register", (req, res) => {
    ...
    db.query(`SELECT * FROM panier WHERE Id_Panier = ? AND Status = "Ouvert"`, [Id_Panier], (error, result) => {
        if (error){
            return(res.status(500).json({message : "Erreur du serveur"}));
        }
        if (result.length === 0){
            return(res.status(404).json({message : "Panier non trouvé ou déjà commandé"}));
        } else {
            db.query(`SELECT * FROM adresse WHERE Numero_Voie = ? AND Nom_Type_Voie = ? AND Nom_commune_Adresse = ? AND Code_postal_Voie = ?`,
                ...
                if (result.length === 0){
                    db.query(`INSERT INTO adresse (Bis_Ter_Numero_de_voie, Nom_commune_Adresse, Nom_Type_Voie, Code_postal_Voie, Numero_Voie, Type_Voie, Adresse_Ville)
                                VALUES (0, ?, ?, ?, ?, ?, ?)`,
                        [Ville, Adresse, Code_Postale, Numero_de_voie, Adresse.split(" ")[0], Code_Postale],
                        (error, result) => {
                            if (error){
                                return(res.status(500).json({message : "Erreur du serveur"}));
                            }
                            Id_Adresse_1 = result.insertId
                            db.query(`INSERT INTO commande (Id_Panier, Id_Adresse_1) VALUES (?, ?)`,
                                [Id_Panier, Id_Adresse_1],
                                (error, result) => {
                                    if (error){
                                        return(res.status(500).json({message: "Erreur lors de la création de la commande"}))
                                    }
                                    res.status(201).json({message : "Commande réussie", Commande_Id : result.insertId})
                                })
                        })
                } else {
